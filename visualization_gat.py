...
def get_site_indices(model, data):
    ...
    """
    x, edge_index, edge_attr = data.x.clone(), data.edge_index, data.edge_attr
    model.eval()
    with torch.no_grad():
        out = model(x, edge_index, edge_attr).detach()
...
if __name__ == "__main__":
    ...
    for i in range(len(node_features)):
        ...
        datalist.append(Data(x=nf, edge_index=ei, edge_attr=ef, res_id=r))
